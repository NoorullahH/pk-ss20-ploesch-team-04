...
public class ChartBuilderForMulContributors extends Application{
...
//			Months.April, Months.May, Months.June, Months.July, Months.August, Months.September, Months.October, Months.November, Months.December);
	...
	//Initializing global variables from constructor param
	List<String> lstFilteredMonth = new ArrayList<String>();
	public ChartBuilderForMulContributors(Months month, FilteredList<Task> filteredData, String startDate, String endDate, String toogleGroupValue) {
	...
	public void start(Stage stage) {
		...
		//If user select month from combo then it shows that month name of title
		if(month != null) {
			barChart.setTitle("Multiple Contributors Tasks of the month "+month);
		}
		//If user do not select month from combo then it shows Only Task
			...
			barChart.setTitle("Multiple Contributors Tasks");
		}
		//If user select only start date then it shows Only start date
			...
			barChart.setTitle("Multiple Contributors Tasks of "+this.filterStartDate);
		}
	...
	public void getOpentaskCount(String month) {
		...
		//If user select month or both dates
			...
			.bind(Bindings.createObjectBinding(() -> filters.stream().reduce(x -> true, Predicate::and), filters));
				...
				filters.add(FilterBuilder.mulContributes());
				filters.add(FilterBuilder.status("open"));
				openTaskCount = filteredData.size();
				initData();
				filters.clear();
